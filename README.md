...
- 实现规格：`.agents/spec/01-ascii-firework-v1.md`（URL、部署、字体、音频、DPR 继续有效）、`.agents/spec/02-sticky-text-reveal.md`（显字算法，取代 spec 01 §7.6 / §7.7 / §8.1）、`.agents/spec/03-directional-ascii-text.md`（方向化 ASCII 文字与完成态托底，取代 spec 02 §4.1 采样与 spec 01 §10 字形清单）、`.agents/spec/04-per-glyph-text-budget.md`（祝福上限改为「每字 64 点、最多 4800 点」预算制，配套收细点阵栅格 `CELL_MAX` 10 / `CELL_MIN` 6：取代设计 01 的 60 字/三行、spec 02 §3.2 `TEXT_SHARE`、spec 03 §4.8-1 的 `budget` 一行与 spec 03 §3 的两个栅格值；`v=1` 单向兼容——旧链接在新版完全等价，新版长链接在旧版被裁到 60 字）
...
另待补验（spec 04 §8 与 spec 03 §11/§12，本地无法执行）：10/60/120/200 字的每字密度均匀性与折行美观、
200 字 + 第十轮收尾 + 连点的真机帧时间（桌面中长祝福峰值 ≈5120 粒子）、
`CELL_MIN` 6 / `CELL_MAX` 10 的可辨性矩阵（`CELL_MIN` 已触及 spec 03 原先记录的可辨性下界）。

## 字体

`public/fonts/ascii-mono.woff2` 是从 **JetBrains Mono 2.304 Regular** 子集化出来的 9 个字形
（`* + . : | / \ @ -`，912 字节），只用于 Canvas 粒子绘制，避免第三方字体请求。
